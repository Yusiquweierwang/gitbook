...
### 10th
- [ ] 问题：
  提取出来的值是筛选的深圳湾沿海的添加了地形水深的点数据， 如果要做需要搭配深圳湾DEM地形来制作；
  或者只选择水深变化值作为



### 12th
- [ ] 江门数据处理内容： 村委会 / 村民委员会 / 人民政府 / 公司/企事业单位/厂矿/日杂的广东省前缀问题9
...
为每一类地理要素预设基准速度（这部分是GPT预生成的，不过也给出了FRUNE J 1971）

询问夏老师： 以CIM/ BIM为建筑标识物做实验 有没有实验对比数据



就是作为毕业论文来说内容也较多，

这里的复杂环境指的就是既指的是室内室外联通场景，也指的是
不同灾情环境，比如火灾，洪水

但是我在考虑要不要添加这么多研究内容。

感觉自己的论文应该着重讲好两部分内容：
一个是CIM和BIM标准融合的行人疏散地理场景构建。
第二个就是基于d*LITE的行人疏散。

我觉得既然是统计受灾情况，就取风暴潮位最高点 - 就近的几个小时来作为数据支撑。



### 23th
- [ ] 韶关数据简化：
1.梦玲姐的 图片中的flmc筛选 + tc = bd + 字段长度 > 13

- 楼栋号 删除 - 前面的内容
- [x] 去掉如“广东省/ xx市 / xx县 / xx镇 ”等前缀
曲江区 / 浈江区/ 武江区/ 乳源瑶族自治县 / 乐昌市 / 仁化县/ 南雄市/ 始兴县 / 翁源县 / 新丰县 / 
- 去掉过长字段
- [x] 去掉“派出所\警务室\公安局\人民政府\村委会\供电所 "等的部分前缀
- 去掉特殊符号 如 点
- 去掉不是主体的括号内容
- [x] 去掉停车场过长字段
...
**生活服务中包含电信 / 移动 / 联通，记得保留，然后删除其他的生活服务内容**


厕所统一修改为厕所
大厦/写字楼 单独删

**生活服务单独处理，或者处理完后把mc 再复制过去**
如包含韶关学院，要还原
...
- [ ] 问红名的问题：
1.道路是否需要整个重庆市范围的道路数据
因为太大了，cityengine里打不开
...
“平急两用”公共基础设施指“平时”满足正常使用功能，“急时”
可转换为应对新发重大疫情和突发公共事件，满足应急隔离、临时安置、
物资保障、医疗救治等需求的公共基础设施。


“平急两用”公共基础设施的设计施工宜采用 BIM（建筑信息
模型）技术，形成完整的模型成果，数据集中妥善储存。运行期间加强
对 BIM 数据模型的运行维护，在应急启用时可迅速实现三维可视化和物
资汇总等功能，实现智慧城市大数据调度功能。


![1743585972624](image/TODO/1743585972624.png)

![1743586037506](image/TODO/1743586037506.png)


![1743586975949](image/TODO/1743586975949.png)
...
- [ ] 判断方式：判断当前水深值的同时，判断下一个水深值的大小，如果（>/<）  当前水深值，则。。。
...
给夏老师检查一下选取时段的水深值是否有代表性。



因为论文中的BIM信息模型需要用到需要用到IFC，而revit 是 BIM 模型导入 UE 的一个常用软件，所以最近有在

如果要在UE中做实验需要把这些模型通过UE datasmith 导入进来，而模型一般用BIM 建模，这些模型的属性

、



- [ ] 要不试试下载一个BIM建筑模型看下里面的属性。

IFC格式的模型可以直接提供模型属性内容到UE，如
防火门状态、逃生通道宽度、楼梯承重能力fe
...
是否考虑不采用cim 而是 indoorGML cityGML

在外文中搜索关键词  indoorGML cityGML

![1744886242232](image/TODO/1744886242232.png)
